...
run = do
  ...
  euBond <- convertibleFixedCouponBond euEx conversionRatio callabilities issue settlementDays (fromList coupons) bdc sched redemption (0, Days) cal Unadjusted False
  amBond <- convertibleFixedCouponBond amEx conversionRatio callabilities issue settlementDays (fromList coupons) bdc sched redemption (0, Days) cal Unadjusted False
