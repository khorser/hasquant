...
spec evalDate = do
    ...
    describe "cash flow leg" $ do
      ...
      it "fixed rate leg as of default settlement date" $ do
        ...
        l <- CF.fixedRateLeg sch (100.0 NE.:| []) (cpn NE.:| []) Following dc cal
      ...
      it "check for segfaulting regression with dynamic cast of coupon in Black pricer" $
        Settings.keepingSettings' $ do
          ...
          cpns <- CF.iborLeg sch index3m (100 NE.:| []) dc Following [2] [] [0.000115] [] [] False False
    ...
    describe "cash flow analytics" $ do
      ...
      it "term-structure NPV analytics: npv vs npv' (zero z-spread), npvbps decomposition, zSpread round-trip, atmRate repricing" $
        Settings.keepingSettings' $ do
          ...
          lAtm <- CF.fixedRateLeg sch2 (100.0 NE.:| []) (cpnAtm NE.:| []) Following dc cal
    ...
    describe "CMS" $ do
      let refDate = 11 `december` 2012
          mkFixture = do
            ...
            let mkLeg = CF.cmsLeg sch swapIdx (1.0 NE.:| []) dc Unadjusted [] [] [] [] [] False False
      ...
      it "digital CMS coupon and leg options affect the priced result" $
        Settings.keepingSettings' $ do
          ...
          let opts = CF.defaultDigitalCmsLegOpts
              ...
              priceLeg legOpts = do
                leg <- CF.digitalCmsLeg sch swapIdx (1.0 NE.:| []) dc Unadjusted [fixingDays] [1.0] [0.0] False legOpts
      ...
      it "CMS and Ibor legs, CMS-rate bonds, and their full options price with effective caps, floors, and amortization" $
        Settings.keepingSettings' $ do
          ...
          let priceCmsLeg caps floors = do
                leg <- CF.cmsLeg sch swapBase (1000000 NE.:| []) thirty360bb Following [2] [1.0] [0.0] caps floors False False
                CF.setCouponPricer leg cmsPricer
                CF.npv leg ts False Nothing Nothing
              priceIborLeg caps floors = do
                leg <- CF.iborLeg sch euribor6m (1000000 NE.:| []) thirty360bb Following [2] [1.0] [0.0] caps floors False False
          ...
          iborFull <- CF.iborLegFull sch euribor6m (1000000 NE.:| []) thirty360bb Following [2] [1.0] [0.0] [] [] False False
          ...
          cmsFull <- CF.cmsLegFull sch swapBase (1000000 NE.:| []) thirty360bb Following [2] [1.0] [0.0] [] [] False False
          ...
          let notionals = [1000000, 500000, 250000, 100000]
              ...
              priceAmortizing ns rs = do
                bond <- Bond.amortizingCmsRateBond 2 (NE.fromList ns) sch swapBase thirty360bb Following 2 [1.0] [0.0] [] [] False Nothing rs
    ...
    describe "HistoricalRatesAnalysis" $
      ...
      it "mean/standardDeviation/min/max match hand-computed values; VaR/ES/gaussian* don't throw and are self-consistent; covariance/correlation are self-consistent for a series against itself" $
        Settings.keepingSettings' $ do
          ...
          hra <- historicalRatesAnalysis startDate (last ds) (1, Months) (idx NE.:| [idx])
