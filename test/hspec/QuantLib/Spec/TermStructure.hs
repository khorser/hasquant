...
spec = do
    ...
    describe "relinkable handles" $ do
      ...
      -- Deposit helpers isolate pillar values from interpolation between pillars, so both
      -- curve types must reprice them to the same discounts at maturity.
      ...
      -- Additional helpers and dates must preserve each deposit's fitted quote.
      ...
      -- ForwardRate/Linear and ZeroYield/Linear must also reprice their input instruments.
