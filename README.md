...
- migrate `shortratemodels` example/tests from QuantLib -- needs, on models already bound (`HullWhite`, `Vasicek`, `ExtendedCoxIngersollRoss`): `OneFactorAffineModel::discountBond`, `HullWhite::r0()`, `HullWhite::convexityBias` (static), `HullWhite::FixedReversion()` constraint + the `calibrate` overload taking a `Constraint`/fixed-params list, and `SwaptionHelper::underlying()`/`swaption()`/`volatility()` plus `CalibratedModel::value()`/`endCriteria()` post-calibration accessors. Also needs `RelinkableHandle<YieldTermStructure>`/`linkTo` (no relinkable handle concept exists in hasquant at all -- blocks 2 of the example's tests specifically; a separate, bigger project, not a small addition)
- migrate `Gaussian1dModels` example from QuantLib -- needs a whole new model/instrument/engine family, not currently bound at all: `Gsr`, `MarkovFunctional`, `Gaussian1dModel` + its swaption engines (`Gaussian1dSwaptionEngine`, `Gaussian1dNonstandardSwaptionEngine`, `Gaussian1dFloatFloatSwaptionEngine`), `NonstandardSwap`/`NonstandardSwaption` (incl. `calibrationBasket`/`BasketGeneratingEngine`), `FloatFloatSwap`/`FloatFloatSwaption`, `RebatedExercise`, `LinearTsrPricer`, and a generic `Instrument::additionalResults()`/`result<T>(name)` extraction mechanism (no analog exists in hasquant). Disproportionate in scope to a single example port -- worth its own project if wanted later.
