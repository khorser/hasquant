...
- `EquityCashFlow`/`EquityIndex`/`EquityTotalReturnSwap` (`ql/cashflows/equitycashflow.hpp`, `ql/indexes/equityindex.hpp`, `ql/instruments/equitytotalreturnswap.hpp`) are not bound -- unlike `ZeroInflationCashFlow`/`CPICashFlow`, this needs a whole new index family (`EquityIndex`, curve/quote-based, not yet bound at all) plus an `EquityCashFlowPricer`/`EquityQuantoCashFlowPricer` pricer hierarchy before the cash flow itself is worth anything, and a full `Swap` subclass (`EquityTotalReturnSwap`, two ctor overloads) to actually consume it -- roughly 30+ tracker lines across three layered pieces, a mid-size new-hierarchy addition rather than a small one. Worth its own follow-up project.
