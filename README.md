...
- `kInterpolatedYoYOptionletVolatilitySurfaceBlack`/`UnitDisplacedBlack`/`Bachelier` (the YoY optionlet vol stripper) hardcode `Linear` for both the per-strike bootstrap interpolation (`InterpolatedYoYOptionletStripper<Interpolator1D>`) and the cross-strike interpolation (`KInterpolatedYoYOptionletVolatilitySurface<Interpolator1D>`), and `yoyCapFloorTermPriceSurface` hardcodes `Bicubic`/`Cubic` for `InterpolatedYoYCapFloorTermPriceSurface<Interpolator2D,Interpolator1D>` — all matching the only combinations upstream's own test-suite (`inflationvolatility.cpp`) exercises. `PiecewiseYoYOptionletVolatilityCurve`/`YoYOptionletHelper`/`YoYOptionletStripper`/`InterpolatedYoYOptionletStripper` are never exposed as their own types: nothing in upstream reaches them from outside `YoYOptionletStripper::initialize`'s own C++ implementation, so a Haskell binding would have nothing to return. Widen any of these only on a concrete need.
...
- `QuantLib.CashFlow.cpiLeg` has no capped/floored variant, unlike `yoyInflationLeg` (which does, via `withCaps`/`withFloors` on the leg builder plus `setYoYInflationCouponPricer`). This isn't an unbound feature: QL 1.43 has no `CappedFlooredCPICoupon` class at all, and `CPICouponPricer` itself is upstream-documented as unfinished for volatility-dependent (i.e. capped/floored) coupons, with no concrete descendant to bind (contrast `YoYInflationCouponPricer`'s three: Black/UnitDisplacedBlack/Bachelier). Revisit if/when upstream QuantLib adds the missing class.
