...
# TODO
- `piecewiseYieldCurve`/`piecewiseYieldCurve'` cover `GlobalBootstrap` only for trait=`Discount` x interpolator=`LogLinear` (`qlPiecewiseYieldCurveAux1`, `cbits/qlTermStructureAux.cpp`), against the full trait x interpolator matrix the `IterativeBootstrap` path supports. This is deliberate, and matches the state of the art rather than lagging it: QuantLib-SWIG (`SWIG/piecewiseyieldcurve.i`) hand-curates ~20 *named* trait x interpolator combos for `IterativeBootstrap` -- not the full matrix either -- and binds exactly **one** `GlobalBootstrap` combination (`GlobalLinearSimpleZeroCurve` = `SimpleZeroYield` x `Linear`), under upstream's own comment "hard-coded to linearly-interpolated, simply-compounded zero rates for now"; `GlobalBootstrap` doesn't appear in the QuantLib-Python docs at all. No binding generator solves this generically -- every one of them curates by hand, because each combination is a distinct template instantiation paid for in compile time and object size, and (for `GlobalBootstrap`) each carries the `[temp.inst]` incomplete-type trap documented in `CLAUDE.md` and at the call site. So: widen mechanically when a real use case asks for a specific combination, and don't design a generic dispatch for the hypothetical rest. See also the `additionalHelpers`/`additionalDates` entry below -- SWIG binds those via canned C++ functor classes (`AdditionalErrors`/`AdditionalDates`) rather than language-level callbacks, which is a cheaper route than the one that entry rules out.
