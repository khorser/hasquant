...
hasquant is a custom wrapper, hand-designed and built directly against QuantLib's C++ API via `c2hs` — it does not use, wrap, or depend on [QuantLib-SWIG](https://github.com/lballabio/QuantLib-SWIG) in any way. Comments and Haddock throughout the codebase do cite QuantLib-SWIG occasionally, purely as a reference point: it's another mature, long-lived per-language QuantLib wrapper that has already made (and documented, in its own `.i` interface files) the same API-shape calls hasquant has to make — which methods are worth exposing, how to shape a callback across the language boundary, which constructor overload of an otherwise-templated class to actually bind. Citing its precedent is a design-input decision, not a code or architecture dependency.
...
- No `yearFraction` binding exists on any day counter, so a golden-value test that needs a specific year fraction has to derive it from `addDays (round (t * basis)) evalDate` instead of computing it directly (see `QuantLib.Spec.Process`/`QuantLib.Spec.Instrument.Option` for the pattern) — add the binding and this workaround goes away
- `G2Process`/`G2ForwardProcess` expose no `phi`/`shortRate`/`factors` accessors, and `HullWhiteForwardProcess` has no `setForwardMeasureTime` binding (a required post-construction call upstream), so neither `G2Process` self-consistency checks nor `HybridHestonHullWhiteProcess` tests are portable from `test-suite/g2process.cpp`/`hybridhestonhullwhiteprocess.cpp` without adding these bindings first
- No `Matrix` object type is bound (only used internally as a raw-array marshalling shape for constructor arguments), and `LfmHullWhiteParameterization`/`CapletVarianceCurve` aren't bound either, so `test-suite/libormarketmodel.cpp::testCapletPricing`'s cached NPV (`0.015853935178`) isn't reproducible — `QuantLib.Spec.Process`'s `LiborForwardModelProcess` coverage checks self-consistency (the new `liborForwardModelProcessFixingDates`/`FixingTimes`/`CashFlows`/`Index` getters agree with each other, and an `AnalyticCapFloorEngine`-priced cap comes out non-negative) with hand-picked vol/correlation inputs instead
- `NoArbSabrModel::absorptionProbability` and QuantLib's internal `detail::D0Interpolator` aren't part of any public API to bind, so `test-suite/noarbsabr.cpp::testAbsorptionMatrix` has no portable equivalent; `testConsistencyWithHagan` is ported (via the new `smileSectionOptionPrice`/`smileSectionDigitalOptionPrice`/`smileSectionDensity` getters) using its own price/digital/density assertions
