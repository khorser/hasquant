...
Use a public capability class for a genuine secondary interface or common functionality implemented by related bound types with identical argument and result types and identical semantics. It is not a namespace workaround or a reason to add blanket constraints, and must not combine operations merely because their upstream names look alike. A multi-parameter capability may encode only the supported coordinate-type combinations as instances; prefer distinct module-qualified functions or a sum-typed argument when signatures or semantics differ.
