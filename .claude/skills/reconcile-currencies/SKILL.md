...
**Gotcha:** editing only `cbits/qlEnumObjects.h`/`cbits/qlMisc.cpp` (no `.chs` file touched) can leave the incremental build silently stale under both `cabal build` and `stack build` — neither tracks that a `.chs` file's `#include`d C header changed, so either may report success without ever re-running c2hs, and a subsequent test run will pass against the *old* generated enum. If in doubt, do a clean build (`cabal clean` / `stack clean`) before rebuilding, and verify end-to-end at the value level (construct one of the new currencies and print something derived from it — e.g. via a script in `smoke/`), not just "the build succeeded."
