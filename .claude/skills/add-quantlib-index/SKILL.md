...
**Case 1 gotcha:** the general `.chs`/header staleness issue (see CLAUDE.md) applies here too — editing only `cbits/qlEnumObjects.h`/`cbits/qlTermStructure.cpp` touches no `.chs` file, so the build can report success without re-running c2hs, and tests then pass against the *old* generated enum. Confirmed by checking the generated `dist-newstyle/.../QuantLib/Index/InterestRate.hs` (or the stack-equivalent `.stack-work/...` path) timestamp/content after such a change. If in doubt, do a clean build before rebuilding, and verify end-to-end at the value level (construct one of the new cases and print something derived from it, e.g. via a `smoke/` script), not just "the build succeeded."
