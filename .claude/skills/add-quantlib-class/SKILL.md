...
4. **`QuantLib/Internal/Type.hs`** — add type and marshalling boilerplate. The shape differs depending on whether this is a **new hierarchy root** or a **class in an existing hierarchy**:
   ...
   **When promoting a former leaf to a `GenX` family, audit every public input that names its concrete `X` alias.** Search direct c2hs imports, hand-written and capability-class signatures, nullable and collection inputs, ADT constructor fields, and generated options-record fields throughout `QuantLib/`. Widen an input to `GenX x` when its C shim takes the `X` base pointer; widen the matching array or `Maybe` marshaller if needed. Give independent inputs distinct type variables. Keep a concrete input only when the upstream operation genuinely requires that exact subclass, and test a newly admitted bound subtype without an explicit `asX` conversion.
