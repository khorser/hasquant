...
- ~~`Money` and `ExchangeRate`/`ExchangeRateManager`~~ — **closed.** `ExchangeRate` (`exchangeRate`/`rate`/`exchangeRateType`/`exchange`/`chainExchangeRate` — `source`/`target` were dropped after review: both just echo the currencies already in the caller's hand from `exchangeRate`'s own arguments or `lookupExchangeRate`'s query, so they're trivial-getter dead weight, not real bindings), `ExchangeRateManager` (`addExchangeRate`/`lookupExchangeRate`/`clearExchangeRates`, bound as free functions like `Settings` — verified via `Singleton<T>` that it's a genuine process-global singleton, not just structurally similar), and `Money::Settings` (`moneyConversionType`/`setMoneyConversionType`/`moneyBaseCurrency`/`setMoneyBaseCurrency`, plus a `convertToBaseCurrency` helper reproducing `ql/money.cpp`'s `convertToBase`) are now all in `QuantLib/Currency.chs`. `Money` itself stays a deliberate `(Double, Currency)` tuple rather than a dedicated type — see the "Out of scope by design" list below, same treatment as `Period`; `exchange`/`convertToBaseCurrency` both take and return that tuple (`withMoney`/`peekCurrencyPtr` in `Internal/Type.hs`, split/joined via c2hs's `&` combinator) for symmetry.
...
- `Period` as a dedicated type — deliberately marshalled as `(Int, TimeUnit)` throughout. Same treatment for `Money`, marshalled as `(Double, Currency)` (see `ExchangeRate` above).
