...
Issue #18 closed the same theme's remaining two hooks, each answered differently after reading what QuantLib actually does internally. `Payoff` genuinely needs a callback — `operator()` is invoked one scalar price at a time everywhere upstream and nothing batches an `Array` — so `withCustomPayoff`/`withCustomBasketPayoff` wrap a Haskell function as a real `Payoff`, usable with every payoff-generic consumer (`oneAssetOption`, `fdmLogInnerValue`, `basketOption`, `mcAmericanEngine` without a control variate) and documented against the engines that downcast and reject it. `StochasticProcess` needs no callback at all: `gaussianRsg`/`nextSequence` expose the gaussian sequence generator `MultiPathGenerator` merely consumes, so a Haskell-defined SDE is evolved in Haskell with zero FFI crossings per timestep — the `lsmRegress` decomposition one level lower — worked in the new `CustomSDE` example.

The upshot: you can now supply your own payoff. Any Haskell `Double -> Double` is a real QuantLib `Payoff`, priced through the FD, basket and Monte Carlo engines.
