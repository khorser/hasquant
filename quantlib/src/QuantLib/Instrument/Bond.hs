...
fixedRateBondFromSchedule :: Word -- ^settlementDays
  ...
  -> Schedule s -- ^schedule
  -> [Double] -- ^coupons
  -> DayCounter s -- ^accrualDayCounter
...
settlementValueFromCleanPrice :: Bond s
  -> Double -- ^cleanPrice
  -> QLE s Double
...
notionals :: Bond s -> QLE s [Double]
notionals b = mkQLE $ map realToFrac <$> withObject b (getArrayX . c_notionals)
...
redemptions :: Bond s -> QLE s (Leg s)
...
cleanPriceFromYield' :: Bond s -- ^bond
  -> InterestRate s -- ^yield
  -> Day -- ^settlementDate
  -> QLE s Double
cleanPriceFromYield' = $(ffiCallX 'cleanPriceFromYield') c_cleanPriceFromYield'
...
nextCashFlowAmount :: Bond s -- ^bond
  -> Day -- ^refDate
  -> QLE s Double
...
previousCashFlowAmount :: Bond s -- ^bond
  -> Day -- ^refDate
  -> QLE s Double
...
referencePeriodEnd :: Bond s -- ^bond
  -> Day -- ^settlementDate
  -> Either QLError (Maybe Day)
...
referencePeriodStart :: Bond s -- ^bond
  -> Day -- ^settlementDate
  -> Either QLError (Maybe Day)
...
yieldFromCleanPrice' :: Bond s -- ^bond
  -> Double -- ^cleanPrice
  -> DayCounter s -- ^dayCounter
  -> Compounding -- ^compounding
  -> Frequency -- ^frequency
...
yieldValueBasisPoint :: Bond s -- ^bond
  -> Double -- ^yield
  -> DayCounter s -- ^dayCounter
  -> Compounding -- ^compounding
  -> Frequency -- ^frequency
...
currentCleanPrice :: Bond s -> QLE s Double
...
currentDirtyPrice :: Bond s -> QLE s Double
