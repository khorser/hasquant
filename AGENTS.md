...
- Use a public capability class for a genuine secondary interface or common functionality implemented by related bound types with the same argument and result types and the same semantics. Keep unrelated APIs concrete, and do not add blanket constraints merely to avoid qualified imports or combine similarly named operations with different meanings. A multi-parameter capability may encode the valid coordinate representations as instances when that prevents unsupported combinations from type-checking.
- Collapse fixed-date and evaluation-date-relative constructors behind `Reference` when their remaining arguments and semantics match. Use `CalendarReference` when both overloads independently require the same calendar. Keep separate `Moving` names when upstream's remaining constructor shape is genuinely different.
- Collapse complete date/time/tenor query families behind the shared coordinate ADTs. Use an interval ADT or a capability class when separate arguments would otherwise admit unsupported mixed coordinate combinations.
