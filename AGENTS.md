...
- Prefer a public capability class over repeated type-prefixed names when related bound types implement the same operation with the same argument and result types and the same semantics. Keep unrelated APIs concrete: do not introduce a class merely to avoid qualified imports or combine similarly named operations with different meanings. A multi-parameter capability may encode the valid coordinate representations as instances when that prevents unsupported combinations from type-checking.
